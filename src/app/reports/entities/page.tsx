...
export default function EntitiesReportPage() {
  ...
  // PAGINATION LOGIC
  // ========================================================================
  const totalPages = Math.ceil(entityArray.length / itemsPerPage);
  const paginatedEntities = entityArray.slice(
              ...
              {paginatedEntities.length === 0 ? (
                ...
                paginatedEntities.map((ent, idx) => (
                  <TableRow key={idx} className="dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
                    <TableCell className="font-medium text-slate-900 dark:text-slate-100">{ent.name}</TableCell>
                    <TableCell className="capitalize text-slate-500">
                      <Badge variant="outline" className="dark:border-slate-700 dark:text-slate-300">{ent.type}</Badge>
                    </TableCell>
                    <TableCell className="text-right font-bold text-slate-900 dark:text-slate-100">
                      {formatMoney(ent.volume, settings.currency)}
                    </TableCell>
                  </TableRow>
                ))
              )}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/30 print:hidden">
            ...
            <div className="flex items-center gap-4 w-full sm:w-auto">
              ...
              <span className="text-sm text-slate-500 dark:text-slate-400 hidden sm:inline-block">
                Showing <span className="font-medium text-slate-900 dark:text-slate-200">{((currentPage - 1) * itemsPerPage) + 1}</span> to <span className="font-medium text-slate-900 dark:text-slate-200">{Math.min(currentPage * itemsPerPage, entityArray.length)}</span> of <span className="font-medium text-slate-900 dark:text-slate-200">{entityArray.length}</span> entries
              </span>
            </div>
